...
class ir_attachment_facturae_mx(osv.Model):
    ...
    def signal_cancel(self, cr, uid, ids, context=None):
        try:
                    ...
                    if not ir_attach_facturae_mx_id.state in ['cancel', 'draft', 'confirmed']:
                        ...
                        if ir_attach_facturae_mx_id.type in type__fc.keys():
                            ...
                            if cfdi_cancel.get('status', True):
                                ...
                                wf_service.trg_validate(
                                    uid, self._name, ir_attach_facturae_mx_id.id, 'action_cancel', cr) #Este es el mtd?
                                ...
                                inv_cancel_status = False
                                cancel_propage = True
                                raise osv.except_osv(_("Invoice not Found!"), _(
                        "Status Invoice:" + (ir_attach_facturae_mx_id.type or '')))
            ...
            if not cancel_propage:
                error = tools.ustr( traceback.format_exc() )
                self.write(cr, uid, ids, {'msj': error}, context=context)
                _logger.error( error )
                return False
            else:
                raise osv.except_osv(_("El comprobante no se puede cancelar en este momento!"), _(
